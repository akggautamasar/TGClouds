        ...
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {files.map((file, index) => (
                <Card key={index} className="group relative overflow-hidden rounded-lg shadow-sm transition-all hover:shadow-md">
                    <Link target="_blank" href={`https://t.me/kuneDrive/${file.id}`} className="absolute inset-1 z-10" prefetch={false}>
...
// function displayImage({ buffer, name, type }: { buffer: Buffer, type: string, name: string }) {
